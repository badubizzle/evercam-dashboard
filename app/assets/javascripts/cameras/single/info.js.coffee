  ...
  onSuccess = (result, status, jqXHR) ->
    ...
    for model in models
      ...
      if model.name.toLowerCase().indexOf('default') isnt -1
        $("#camera-model").prepend("<option jpg-val='#{jpg_url}' rtsp-val='#{model.h264_url}' value='#{model.id}' #{selected}>#{model.name}</option>")
      else if model.name.toLowerCase().indexOf('other') isnt -1
        $("#camera-model").prepend("<option jpg-val='#{jpg_url}' rtsp-val='#{model.h264_url}' value='#{model.id}' selected='selected'>#{model.name} - Custom URL</option>")
      else
        $("#camera-model").append("<option jpg-val='#{jpg_url}' rtsp-val='#{model.h264_url}' value='#{model.id}' #{selected}>#{model.name}</option>")
